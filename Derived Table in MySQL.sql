...
-- top 5 balances
SELECT cur_balance FROM account_details
ORDER BY cur_balance DESC
LIMIT 5;

-- step 2 finding top 5th balance 
SELECT MIN(cur_balance) FROM(
      SELECT cur_balance FROM account_details
      ORDER BY cur_balance DESC
      LIMIT 5) AS top_balance;

-- * finding 2nd highest balance
SELECT MIN(cur_balance) FROM(
      SELECT cur_balance FROM account_details
      ORDER BY cur_balance DESC
      LIMIT 2) AS top_balance;